...
import 'package:My_Wallet/ui_view/body_measurement.dart';
import 'package:My_Wallet/ui_view/mediterranean_diet_view.dart';
import 'package:My_Wallet/ui_view/title_view.dart';
...
class _MyDiaryScreenState extends State<MyDiaryScreen> with TickerProviderStateMixin {
  ...
  List<dynamic> _operationsList = <dynamic>[];
  late int _walletType = 0;
  ...
  Future<void> addAllListData() async {
    ...
    for (var i = 0; i < _walletType; i++) {
      listViews.add(
        TitleView(
          titleTxt: _operationsList[i]["wallet"].toString(),
          subTxt: 'Details',
      ...
      listViews.add(
        BodyMeasurementView(
          ...
          animationController: widget.animationController!,
          totalSpend: await db.getTypeAmount(_operationsList[i]["wallet"]),
          lastDateTime: await db.getTypeDate(_operationsList[i]["wallet"]),
        ),
      );
    }
  }
  ...
  Future<bool> getData() async {
    ...
    var result;
    List<dynamic> listData = await db.getType();
    if (listData.isEmpty) {
  ...
  readOperationsList() async {
    ...
    i = await db.getTypeTotal();
    listData = await db.getType();
    if (!mounted) return;
    setState(() {
      _walletType = i;
      _operationsList = listData;
    });
    addAllListData();
  }
}
